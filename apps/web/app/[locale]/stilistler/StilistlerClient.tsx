...
export function StilistlerClient({ stylists }: { stylists: Stylist[] }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
        ...
        <form onSubmit={applyFilters} style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            ...
            <input
              ...
              style={{ height: 44, boxSizing: 'border-box', padding: '0 1rem', borderRadius: 8, border: '1px solid #e4d4f4', outline: 'none', fontSize: '0.9rem' }}
            />
          </div>
          ...
          <div style={{ flex: '1 1 150px', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            ...
            {cities.length > 0 ? (
              <select
                ...
                style={{ height: 44, boxSizing: 'border-box', padding: '0 1rem', borderRadius: 8, border: '1px solid #e4d4f4', outline: 'none', fontSize: '0.9rem', backgroundColor: 'white' }}
              >
          ...
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flex: '1 1 200px' }}>
            <button
              type="submit"
              style={{ flex: 1, height: 44, boxSizing: 'border-box', background: '#7c3aed', color: 'white', border: 'none', padding: '0 1.5rem', borderRadius: 8, fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer' }}
            >
              Filtrləri tətbiq et
            </button>
            <button
              ...
              style={{ background: 'none', border: 'none', color: '#7c6fa0', textDecoration: 'underline', fontSize: '0.9rem', fontWeight: 500, cursor: 'pointer', padding: 0, whiteSpace: 'nowrap' }}
            >
              Sıfırla
            </button>
          </div>
      ...
      {/* Results count */}
      {stylists.length > 0 && (
        <p style={{ color: '#7c6fa0', fontSize: '0.9rem', margin: '0 0 1.5rem 0' }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', color: '#7c6fa0' }}>
          <p style={{ fontSize: '1.1rem', margin: '0 0 0.5rem 0', color: '#1e1b2e' }}>Stilist tapılmadı</p>
              ...
              >
                ...
                <div style={{ padding: '2rem 1.25rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '0.8rem', color: '#7c6fa0', margin: 0, display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  ...
                  <div style={{ marginTop: 'auto', paddingTop: '0.75rem', display: 'flex', gap: '0.5rem' }}>
                    <button
                      ...
                      style={{ flex: 1, padding: '0.6rem', background: 'transparent', border: '1px solid #e4d4f4', color: '#1e1b2e', borderRadius: 8, fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer' }}
                    >
                      Portfolio
                    </button>
                    <Link
                      href={`/salons/${s.salon.slug}`}
                      style={{ padding: '0.6rem 1rem', background: '#7c3aed', color: 'white', borderRadius: 8, fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none', textAlign: 'center' }}
                    >
                      Rezerv et
                    </Link>
                  </div>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '1.75rem 2rem 1.25rem', borderBottom: '1px solid #e4d4f4' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: '#7c6fa0', margin: '0 0 1rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              ...
              {selectedStylist.bio && (
                <div style={{ fontSize: '0.9rem', color: '#6b5d8a', lineHeight: 1.6, background: '#f9f6f3', padding: '0.85rem 1rem', borderRadius: 10, border: '1px solid #e4d4f4' }}>
                  {selectedStylist.bio}
                </div>
              )}
            </div>
            ...
            <div style={{ padding: '1rem 2rem calc(1rem + env(safe-area-inset-bottom))', borderTop: '1px solid #e4d4f4', display: 'flex', gap: '0.75rem' }}>
              <button
                onClick={() => setSelectedStylist(null)}
                style={{ flex: 1, padding: '0.8rem', background: 'transparent', border: '1px solid #e4d4f4', color: '#7c6fa0', borderRadius: 10, fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer' }}
              >
                Bağla
              </button>
              <Link
                href={`/salons/${selectedStylist.salon.slug}`}
                style={{ flex: 2, padding: '0.8rem', background: '#7c3aed', color: 'white', borderRadius: 10, fontSize: '0.9rem', fontWeight: 600, textDecoration: 'none', textAlign: 'center', boxShadow: '0 4px 12px rgba(92,61,40,0.2)' }}
              >
