        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-3 items-center">
              <Badge tone={stylist.isActive ? 'success' : 'neutral'}>
                {stylist.isActive ? 'Aktiv' : 'Deaktiv'}
              </Badge>
              <Link href={`/superadmin/stylists/${stylist.id}/edit`} className="text-sm">
                Redaktə et
              </Link>
              <Button
                variant={stylist.isActive ? 'destructive' : 'secondary'}
          ...
          <dl className="grid gap-2 text-sm sm:grid-cols-2">
            {(
              [
                [
                  'Salon',
                  <Link key="s" href={`/superadmin/salons/${stylist.salon.id}`}>
                    {stylist.salon.name}
                  </Link>,
                ],
          ...
          <Card>
            ...
            <div className="flex flex-col gap-1.5">
              {stylist.workingSchedules.map((ws) => (
                <div
                  key={ws.id}
                  className="flex items-center justify-between rounded-[var(--radius-sm)] bg-surface px-3 py-2 text-sm"
                >
                  <span className="text-text-secondary">{DAYS_AZ[ws.weekday] ?? ws.weekday}</span>
                  <span className="font-medium text-text-primary">
                    {minToTime(ws.startMinuteOfDay)} – {minToTime(ws.endMinuteOfDay)}
                  </span>
                </div>
              ))}
            </div>
...
function PortfolioTab({ stylist, onReload }: { stylist: StylistDetail; onReload: () => void }) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
      showToast('Yalnız JPEG, PNG, WEBP şəkillər qəbul edilir.', 'danger');
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          {items.map((item, index) => (
            ...
            >
              ...
              {editingId === item.id ? (
                ...
                <button
                  type="button"
                  className="truncate text-left text-xs text-text-secondary hover:underline"
                  onClick={() => {
      ...
      <ConfirmDialog
        ...
        title="Bu şəkli silmək istəyirsiniz?"
      ...
      {assigned.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {assigned.map((svc) => (
            <li
              key={svc.id}
              className="flex items-center justify-between rounded-[var(--radius-md)] border border-border px-4 py-3"
            >
              <span className="text-sm font-medium text-text-primary">{svc.name}</span>
              <IconButton
                label={`Sil: ${svc.name}`}
                icon={<span aria-hidden="true">✕</span>}
                onClick={() => setUnassignTarget(svc)}
              />
            </li>
          ))}
        </ul>
        ...
        tabs={[
          {
            value: 'profile',
            label: 'Profil',
            content: <ProfileTab stylist={stylist} onReload={load} />,
          },
          {
            value: 'portfolio',
            label: `Portfolio (${stylist.portfolio.length})`,
            content: <PortfolioTab stylist={stylist} onReload={load} />,
          },
          {
            value: 'services',
            label: `Xidmətlər (${stylist.services.length})`,
            content: <ServicesTab stylist={stylist} onReload={load} />,
          },
        ]}
      />
    </main>
