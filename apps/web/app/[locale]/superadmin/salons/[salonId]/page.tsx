...
const STATUS_AZ: Record<string, string> = {
  ...
  CANCELLED_BY_CUSTOMER: 'Müş. ləğv',
...
function Avatar({ url, name, size = 32 }: { url: string | null; name: string; size?: number }) {
  return url ? (
    ...
    <span
      className="inline-flex items-center justify-center rounded-full bg-surface text-text-secondary font-semibold"
      style={{
        width: size,
        height: size,
        fontSize: size * 0.4,
...
function OverviewTab({ salon, onReload }: { salon: SalonDetail; onReload: () => void }) {
  ...
  const fields: [string, string | null | number][] = [
    ['Slug', salon.slug],
    ['Şəhər', salon.city],
    ['Vaxt zonası', salon.timezone],
    ['Ünvan', salon.addressLine],
    ['Telefon', salon.phone],
    ['E-poçt', salon.email],
    ['Subdomain', salon.subdomain],
    ['Xüsusi domen', salon.customDomain],
    ['Cinsiyyət fokus', salon.genderFocus],
    ['Aktiv işçilər', salon.activeMembershipCount],
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href={`/superadmin/salons/${salon.id}/edit`} className="text-sm">
              Redaktə et
            </Link>
            <Button
        ...
        <dl className="grid gap-2 text-sm sm:grid-cols-2">
          {fields.map(([label, value]) => (
            <div
              key={label}
              className="flex justify-between gap-4 rounded-[var(--radius-sm)] bg-surface px-3 py-2"
            >
      ...
      <ConfirmDialog
        ...
        title={
          isActive ? 'Salonu deaktiv etmək istəyirsiniz?' : 'Salonu aktivləşdirmək istəyirsiniz?'
        }
        description={
          isActive
...
function StylistsTab({ salonId }: { salonId: string }) {
  ...
  const [state, setState] = useState<LS<{ items: Stylist[]; total: number; pageSize: number }>>({
    kind: 'loading',
  });
    ...
    <div className="flex flex-col gap-4">
        ...
        <>
          {state.data.items.length === 0 ? (
            <EmptyState
              ...
              description="Axtarışı dəyişin və ya yeni stilist əlavə edin."
            />
          ) : (
            <Table
              columns={[
                  ...
                  render: (r: Stylist) => (
                    <DropdownMenu
                      ...
                      items={[
                        {
                          label: 'Profilə keç',
                          onSelect: () => (window.location.href = `/superadmin/stylists/${r.id}`),
                        },
                        {
                          label: 'Redaktə et',
                          onSelect: () =>
                            (window.location.href = `/superadmin/stylists/${r.id}/edit`),
                        },
                        {
                          label: r.isActive ? 'Deaktiv et' : 'Aktivləşdir',
                          onSelect: () => setConfirmTarget(r),
          ...
          <Pagination
            page={page}
            pageCount={Math.max(1, Math.ceil(state.data.total / state.data.pageSize))}
      ...
      {confirmTarget && (
        <ConfirmDialog
          ...
          description={
            confirmTarget.isActive
              ? 'Yeni rezervasiyalar üçün əlçatmaz olacaq.'
              : 'Yenidən rezervasiya qəbul edəcək.'
          }
...
function ServicesTab({ salonId }: { salonId: string }) {
  ...
  const [status, setStatus] = useState('');
  const [page, setPage] = useState(1);
  const [state, setState] = useState<LS<{ items: Service[]; total: number; pageSize: number }>>({
    kind: 'loading',
  });
              ...
              columns={[
                {
                  ...
                  header: 'Ad',
                  render: (r: Service) => (
                    <Link href={`/superadmin/services/${r.id}`} className="font-medium">
                      {r.name}
                    </Link>
                  ),
                },
                { key: 'cat', header: 'Kateqoriya', render: (r: Service) => r.categoryName ?? '—' },
                {
                  ...
                  header: 'Qiymət',
                  render: (r: Service) => (
                    <span className="font-medium">{fmt(r.priceAmount, r.currency)}</span>
                  ),
                },
                {
                  key: 'dur',
                  header: 'Müddət',
                  render: (r: Service) => `${r.durationMinutes} dəq`,
                },
                {
                  key: 'status',
                  header: 'Status',
                      ...
                      items={[
                        {
                          label: 'Redaktə et',
                          onSelect: () =>
                            (window.location.href = `/superadmin/services/${r.id}/edit`),
                        },
                        {
                          label: r.isActive ? 'Deaktiv et' : 'Aktivləşdir',
                          onSelect: () => setConfirmTarget(r),
...
function CategoriesTab({ salonId }: { salonId: string }) {
  ...
  async function toggleActive(cat: Category) {
    try {
      await apiFetch(
        `/salons/${salonId}/service-categories/${cat.id}/${cat.isActive ? 'deactivate' : 'activate'}`,
        {
          method: 'POST',
          body: JSON.stringify({}),
        },
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {state.kind === 'error' && (
        <ErrorState title="Kateqoriyalar yüklənmədi" description={state.message} />
      )}
      {state.kind === 'ready' &&
        (state.data.length === 0 ? (
          <EmptyState
            title="Kateqoriya tapılmadı"
            description="Yuxarıdakı formla ilk kateqoriyanı yaradın."
          />
        ) : (
...
function ReservationsTab({ salonId }: { salonId: string }) {
  ...
  const [status, setStatus] = useState('');
  const [page, setPage] = useState(1);
  const [state, setState] = useState<LS<{ items: Reservation[]; total: number; pageSize: number }>>(
    { kind: 'loading' },
  );
  ...
  const load = useCallback(() => {
    ...
    if (search) q.set('search', search);
    if (status) q.set('status', status);
    apiFetch<{ items: Reservation[]; total: number; pageSize: number }>(
      `/superadmin/reservations?${q}`,
    )
      .then((data) => setState({ kind: 'ready', data }))
      .catch((err: unknown) =>
        setState({ kind: 'error', message: err instanceof ApiError ? err.message : 'Xəta' }),
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-3 flex-wrap">
        ...
        >
          <option value="">Bütün statuslar</option>
          {Object.entries(STATUS_AZ).map(([v, l]) => (
              ...
              columns={[
                {
                  key: 'date',
                  header: 'Tarix',
                  render: (r: Reservation) => new Date(r.startAt).toLocaleDateString('az-AZ'),
                },
                {
                  key: 'time',
                  header: 'Saat',
                  render: (r: Reservation) =>
                    new Date(r.startAt).toLocaleTimeString('az-AZ', {
                      hour: '2-digit',
                      minute: '2-digit',
                    }),
                },
                {
                  key: 'customer',
                  header: 'Müştəri',
                  render: (r: Reservation) => <span className="font-medium">{r.customerName}</span>,
                },
                { key: 'service', header: 'Xidmət', render: (r: Reservation) => r.serviceName },
                {
                  key: 'employee',
                  header: 'Stilist',
                  render: (r: Reservation) => r.employeeName ?? '—',
                },
                {
                  key: 'price',
                  header: 'Məbləğ',
                  render: (r: Reservation) => fmt(r.priceAmount, r.currency),
                },
                {
                  key: 'status',
                  header: 'Status',
    ...
    <main className="dashboard-page">
      ...
      <div className="page-header">
        <div className="min-w-0">
          <Breadcrumbs
            items={[{ label: 'Salonlar', href: '/superadmin/salons' }, { label: salon.name }]}
          />
        ...
        tabs={[
          {
            value: 'overview',
            label: 'Ümumi',
            content: <OverviewTab salon={salon} onReload={load} />,
          },
          ...
          { value: 'services', label: 'Xidmətlər', content: <ServicesTab salonId={salonId} /> },
          {
            value: 'categories',
            label: 'Kateqoriyalar',
            content: <CategoriesTab salonId={salonId} />,
          },
          {
            value: 'reservations',
            label: 'Rezervasiyalar',
            content: <ReservationsTab salonId={salonId} />,
          },
        ]}
      />
    </main>
