...
interface StylistRow {
  employeeId: string; fullName: string; isActive: boolean; photoUrl: string | null;
  salonId: string; salonName: string; totalReservations: number; completedCount: number;
  cancelledCount: number; noShowCount: number; revenue: number; currency: string;
}
interface Response { items: StylistRow[]; total: number; page: number; pageSize: number; }
  ...
  return (
    <main className="dashboard-page">
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>Hər stilist üzrə performans</p>
      </div>
      ...
      {loading && !data ? <Skeleton className="h-64 w-full" /> : data?.items.length === 0 ? (
        <EmptyState title="Məlumat yoxdur" description="Seçilmiş dövrdə stilist məlumatı tapılmadı." />
      ) : data ? (
        <>
          <Table
            columns={[
              { key: 'name', header: 'Stilist', render: (r: StylistRow) => (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--color-accent-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'var(--color-accent)', fontSize: '0.8rem', flexShrink: 0, overflow: 'hidden' }}>
              ...
              { key: 'salon', header: 'Salon', render: (r: StylistRow) => <Link href={`/superadmin/salons/${r.salonId}`}>{r.salonName}</Link> },
              { key: 'total', header: 'Rezervasiya', render: (r: StylistRow) => r.totalReservations },
              { key: 'completed', header: 'Tamamlanan', render: (r: StylistRow) => r.completedCount },
              ...
              { key: 'revenue', header: 'Gəlir', render: (r: StylistRow) => fmt(r.revenue, r.currency) },
            ]}
            rows={data.items}
