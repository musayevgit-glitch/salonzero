...
interface ReservationItem {
  id: string; status: string; startAt: string; priceAmount: number; currency: string;
  customerName: string; salonId: string; salonName: string;
  serviceId: string; serviceName: string; employeeId: string; employeeName: string;
}
interface Response { items: ReservationItem[]; total: number; page: number; pageSize: number; }

const STATUS_LABELS: Record<string, string> = {
  PENDING: 'Gözləyir', CONFIRMED: 'Təsdiqlənib', REJECTED: 'Rədd edilib',
  CANCELLED_BY_CUSTOMER: 'Müştəri ləğv', CANCELLED_BY_SALON: 'Salon ləğv',
  CHECKED_IN: 'Gəldi', COMPLETED: 'Tamamlandı', NO_SHOW: 'Gəlmədi',
};
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1.5rem 2rem' }}>
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>Bütün salonlar üzrə rezervasiya analitikası</p>
      </div>
      ...
      {loading && !data ? <Skeleton className="h-64 w-full" /> : data?.items.length === 0 ? (
        ...
        <>
          <Table
            columns={[
              { key: 'date', header: 'Tarix', render: (r: ReservationItem) => new Date(r.startAt).toLocaleString('az-AZ', { dateStyle: 'short', timeStyle: 'short' }) },
              { key: 'customer', header: 'Müştəri', render: (r: ReservationItem) => r.customerName },
