    ...
    <main style={{ display: 'flex', flexDirection: 'column', gap: '2rem', padding: '1.5rem 2rem' }}>
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>
          Bütün salonlar üzrə məcmu statistika
        </p>
      </div>
      ...
      {data && (
        <>
          ...
          <section>
            ...
            >
              ...
              <StatCard
                ...
                sub={pct(data.reservations.completionRate)}
              />
              <StatCard label="Təsdiqlənənlər" value={data.reservations.confirmed} />
              <StatCard
                label="Ləğv edilənlər"
                value={data.reservations.cancelled}
                sub={pct(data.reservations.cancellationRate)}
              />
          ...
          <section>
            ...
            >
              <StatCard
                label="Ümumi salonlar"
                value={data.salons.total}
                sub={`${data.salons.active} aktiv`}
              />
              <StatCard
                label="Ümumi stilistlər"
                value={data.stylists.total}
                sub={`${data.stylists.active} aktiv`}
              />
              <StatCard
                label="Ümumi xidmətlər"
                value={data.services.total}
                sub={`${data.services.active} aktiv`}
              />
            </div>
          </section>
          ...
          {data.revenue > 0 && (
            <section>
              ...
              >
                <StatCard
                  label="Ümumi gəlir (qəpik)"
                  value={data.revenue.toLocaleString('az-AZ')}
                />
              </div>
            </section>
          )}
