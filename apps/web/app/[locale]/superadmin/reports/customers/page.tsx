...
interface CustomerRow { customerId: string; fullName: string; email: string; phone: string | null; memberSince: string | null; bookingCount: number; }
interface Summary { totalAllTime: number; uniqueInPeriod: number; newInPeriod: number; returningInPeriod: number; }
interface Response { items: CustomerRow[]; total: number; page: number; pageSize: number; summary: Summary; }
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1.5rem 2rem' }}>
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>Müştəri aktivliyi və statistika</p>
      </div>
      ...
      {loading && !data ? <Skeleton className="h-64 w-full" /> : data?.items.length === 0 ? (
        <EmptyState title="Müştəri tapılmadı" description="Seçilmiş dövrdə müştəri məlumatı yoxdur." />
      ) : data ? (
        <>
          <Table
            columns={[
              { key: 'name', header: 'Müştəri', render: (r: CustomerRow) => <div><div style={{ fontWeight: 500 }}>{r.fullName}</div><div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>{r.email}</div></div> },
              { key: 'phone', header: 'Telefon', render: (r: CustomerRow) => r.phone ?? '—' },
              { key: 'since', header: 'Qeydiyyat', render: (r: CustomerRow) => r.memberSince ? new Date(r.memberSince).toLocaleDateString('az-AZ') : '—' },
              { key: 'bookings', header: 'Rezervasiya', render: (r: CustomerRow) => <strong>{r.bookingCount}</strong> },
            ]}
            rows={data.items}
