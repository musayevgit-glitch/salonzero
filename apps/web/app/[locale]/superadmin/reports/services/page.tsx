    ...
    <main className="dashboard-page">
      ...
      {loading && !data ? (
        ...
        <EmptyState
          ...
          description="Seçilmiş dövrdə xidmət məlumatı tapılmadı."
        />
      ) : data ? (
        <>
          <Table
            columns={[
              ...
              },
              {
                key: 'category',
                header: 'Kateqoriya',
                render: (r: ServiceRow) => r.categoryName ?? '—',
              },
              {
                key: 'price',
                header: 'Qiymət',
                render: (r: ServiceRow) => fmt(r.priceAmount, r.currency),
              },
              {
                key: 'duration',
                header: 'Müddət',
                render: (r: ServiceRow) => `${r.durationMinutes} dəq`,
              },
              {
                key: 'stylists',
                header: 'Stilistlər',
                render: (r: ServiceRow) => r.assignedStylistCount,
              },
              { key: 'bookings', header: 'Rezerv.', render: (r: ServiceRow) => r.totalBookings },
              {
                key: 'completed',
                header: 'Tamamlanan',
                render: (r: ServiceRow) => r.completedBookings,
              },
              { key: 'cancelled', header: 'Ləğv', render: (r: ServiceRow) => r.cancelledCount },
              {
                key: 'revenue',
                header: 'Gəlir',
                render: (r: ServiceRow) => fmt(r.revenue, r.currency),
              },
              {
                key: 'status',
                header: '',
