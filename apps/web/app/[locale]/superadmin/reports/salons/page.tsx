...
interface SalonRow {
  salonId: string; salonName: string; status: string; city: string | null;
  activeStylists: number; activeServices: number; totalReservations: number;
  cancelledCount: number; noShowCount: number; cancellationRate: number; noShowRate: number;
  revenue: number; currency: string; confirmedCount: number;
}
interface Response { items: SalonRow[]; total: number; page: number; pageSize: number; from: string; to: string; }
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1.5rem 2rem' }}>
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>Hər salon üzrə performans</p>
      </div>
      ...
      {loading && !data ? (
        ...
        <EmptyState title="Məlumat yoxdur" description="Seçilmiş dövrdə salon məlumatı tapılmadı." />
      ) : data ? (
        <>
          <div style={{ overflowX: 'auto' }}>
            <Table
              columns={[
                { key: 'salon', header: 'Salon', render: (r: SalonRow) => <Link href={`/superadmin/salons/${r.salonId}`}>{r.salonName}</Link> },
                { key: 'city', header: 'Şəhər', render: (r: SalonRow) => r.city ?? '—' },
                { key: 'status', header: 'Status', render: (r: SalonRow) => <Badge tone={r.status === 'ACTIVE' ? 'success' : 'neutral'}>{r.status === 'ACTIVE' ? 'Aktiv' : 'Deaktiv'}</Badge> },
                { key: 'stylists', header: 'Aktiv stilist', render: (r: SalonRow) => r.activeStylists },
                { key: 'services', header: 'Aktiv xidmət', render: (r: SalonRow) => r.activeServices },
                { key: 'reservations', header: 'Rezervasiya', render: (r: SalonRow) => r.totalReservations },
                { key: 'cancelled', header: 'Ləğv', render: (r: SalonRow) => `${r.cancelledCount} (${(r.cancellationRate * 100).toFixed(0)}%)` },
                { key: 'noshow', header: 'Gəlməyən', render: (r: SalonRow) => `${r.noShowCount} (${(r.noShowRate * 100).toFixed(0)}%)` },
                { key: 'revenue', header: 'Gəlir', render: (r: SalonRow) => fmt(r.revenue, r.currency) },
              ]}
              rows={data.items}
