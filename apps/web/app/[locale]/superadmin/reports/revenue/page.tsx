    ...
    <main className="dashboard-page">
      <div>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: 2 }}>
          Salon üzrə gəlir analitikası (rezervasiya qiymət snapshotları əsasında)
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: '1.5rem', fontWeight: 700, margin: '0.25rem 0 0' }}>
              {totalRevenue.toLocaleString()} qəp
            </p>
          </div>
          <div
            style={{
      ...
      ) : data?.bySalon.length === 0 ? (
        <EmptyState
          title="Gəlir məlumatı yoxdur"
          description="Seçilmiş dövrdə tamamlanan rezervasiya tapılmadı."
        />
      ) : data ? (
        <div
          style={{
            ...
            <tbody>
              {data.bySalon.map((row, i) => (
                <tr
                  ...
                  style={{
                    borderBottom:
                      i < data.bySalon.length - 1 ? '1px solid var(--color-border)' : undefined,
                  }}
                >
