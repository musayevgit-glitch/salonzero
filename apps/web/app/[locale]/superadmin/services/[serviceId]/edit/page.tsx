...
interface ServiceDetail { id: string; name: string; description: string | null; priceAmount: number; currency: string; durationMinutes: number; bufferMinutes: number; isActive: boolean; categoryId?: string | null; salon: { id: string; name: string }; }
...
export default function EditServicePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isNaN(price) || price < 0) { setError('Düzgün qiymət daxil edin.'); return; }
    ...
    try {
      await apiFetch(`/superadmin/services/${serviceId}`, {
        method: 'PATCH',
        body: JSON.stringify({ name, description: description || null, categoryId: categoryId || null, priceAmount: price, currency, durationMinutes: dur, bufferMinutes: parseInt(bufferMinutes) || 0, isActive }),
      });
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1.5rem 2rem' }}>
      ...
      <Card style={{ maxWidth: 560 }}>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginBottom: '1rem' }}>Salon: <strong>{service.salon.name}</strong></p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }} noValidate>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <FormField label="Valyuta">
              {(p) => <Select {...p} value={currency} onChange={(e) => setCurrency(e.target.value)}><option value="AZN">AZN</option><option value="USD">USD</option><option value="EUR">EUR</option></Select>}
            </FormField>
          </div>
