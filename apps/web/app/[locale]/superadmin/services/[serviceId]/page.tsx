...
export default function ServiceDetailPage() {
    ...
    apiFetch<ServiceDetail>(`/superadmin/services/${serviceId}`)
      ...
      .catch((err: unknown) => {
        if (err instanceof ApiError && err.status === 401) {
          router.replace(`/login?returnTo=/superadmin/services/${serviceId}`);
          return;
        }
  ...
  async function handleToggle() {
    ...
    try {
      await apiFetch(`/superadmin/services/${serviceId}`, {
        method: 'PATCH',
        body: JSON.stringify({ isActive: !state.service.isActive }),
      });
  ...
  return (
    <main className="dashboard-page">
      <Breadcrumbs
        items={[{ label: 'Xidmətlər', href: '/superadmin/services' }, { label: service.name }]}
      />
        ...
        <Card>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <dd>
                <Link href={`/superadmin/salons/${service.salon.id}`}>{service.salon.name}</Link>
              </dd>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Card>
          ...
          {service.assignedStylists.length === 0 ? (
            <p style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)' }}>
              ...
              <Link href={`/salon/${service.salon.id}/services/${service.id}`}>
                Salon panelindən təyin edin →
              </Link>
            </p>
          ) : (
