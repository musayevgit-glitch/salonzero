...
export async function POST(
  ...
  await prisma.employeeProfile.update({ where: { id: employeeId }, data: { photoUrl: result.url } });
  await recordAudit({ actorUserId: check.userId, action: 'stylist.photo_updated', targetType: 'EmployeeProfile', targetId: employeeId, salonId: employee.salonId });
...
export async function DELETE(
  ...
  await recordAudit({ actorUserId: check.userId, action: 'stylist.photo_removed', targetType: 'EmployeeProfile', targetId: employeeId, salonId: employee.salonId });
