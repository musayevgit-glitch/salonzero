...
export async function GET(req: NextRequest) {
  ...
  const [totalGroups, completedGroups, cancelGroups, noShowGroups, revenueGroups] = await Promise.all([
    ...
    prisma.reservation.groupBy({
      by: ['employeeId'],
      where: { employeeId: { in: empIds }, status: ReservationStatus.COMPLETED, startAt: { gte: start, lte: end } },
      _count: { id: true },
    }),
    prisma.reservation.groupBy({
      by: ['employeeId'],
    ...
    prisma.reservation.groupBy({
      by: ['employeeId'],
      where: { employeeId: { in: empIds }, status: ReservationStatus.NO_SHOW, startAt: { gte: start, lte: end } },
      _count: { id: true },
    }),
    prisma.reservation.groupBy({
      by: ['employeeId', 'currency'],
      where: { employeeId: { in: empIds }, status: { in: REVENUE_STATUSES }, startAt: { gte: start, lte: end } },
  ...
  const rows = employees.map((e) => {
    ...
    const completed = (completedMap[e.id] as { _count: { id: number } } | undefined)?._count.id ?? 0;
