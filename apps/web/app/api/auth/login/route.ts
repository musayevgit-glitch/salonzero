...
export async function POST(req: NextRequest) {
  ...
  if (!user || user.status !== 'ACTIVE' || !(await verifyPassword(user.passwordHash, password))) {
    await recordAudit({ action: 'user.login_failed', targetType: 'User', targetId: user?.id ?? 'unknown' });
    return NextResponse.json({ message: 'Invalid email or password.' }, { status: 401 });
  }

  await recordAudit({ actorUserId: user.id, action: 'user.login_succeeded', targetType: 'User', targetId: user.id });

  const token = signJwt({ sub: user.id, email: user.email, fullName: user.fullName, isSuperadmin: user.isSuperadmin });
  const res = NextResponse.json({ id: user.id, email: user.email, fullName: user.fullName, isSuperadmin: user.isSuperadmin });
